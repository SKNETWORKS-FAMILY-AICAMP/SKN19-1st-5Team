...
with col1:
    ...
    with mini_col:
        st.page_link("pages/01_Chart.py", label="👉 차트 페이지로 이동")

    _, mini_col, _ = st.columns(3)
    with mini_col:
        st.page_link("pages/02_Analysis.py", label="👉  분석 페이지로 이동")

    _, mini_col, _ = st.columns(3)
    with mini_col:
        st.page_link("pages/03_Map.py", label="👉  충전소 지도로 이동")

    _, mini_col, _ = st.columns(3)
    with mini_col:
        st.page_link("pages/04_FAQ.py", label="👉  FAQ로 이동")
