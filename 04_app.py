...
apply_filter = st.sidebar.checkbox("필터 사용", value=False)

if apply_filter:
    orgs = st.sidebar.multiselect("운영기관 선택", options=df["운영기관"].unique())
    types = st.sidebar.multiselect("충전기 타입 선택", options=df["충전기타입"].unique())

    if orgs:
        df = df[df["운영기관"].isin(orgs)]
    if types:
        df = df[df["충전기타입"].isin(types)]
